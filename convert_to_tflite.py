...
from huseyin_dataset import create_dataset
...
def convert(model_settings, checkpoint, quantize, inference_type, tflite_path):
...
def main():
    ...
    if FLAGS.quantize:
        tflite_path = f'{FLAGS.model_architecture}_quantized_{FLAGS.inference_type}.tflite'
    else:
        tflite_path = f'{FLAGS.model_architecture}_no_quantize.tflite'

    # Load floating point model from checkpoint and convert it.
    convert(model_settings, FLAGS.checkpoint,
            FLAGS.quantize, FLAGS.inference_type, tflite_path)

    # Test the newly converted model on the test set.
    tflite_test(model_settings, tflite_path)
