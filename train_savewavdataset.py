...
def train():
    ...
    ds_numpy = tfds.as_numpy(train_data)  # Convert `tf.data.Dataset` to Python generator
    os.makedirs("processed_dataset", exist_ok=True)
    os.makedirs("processed_dataset/class_0", exist_ok=True)
    os.makedirs("processed_dataset/class_1", exist_ok=True)
    os.makedirs("processed_dataset/class_2", exist_ok=True)
    os.makedirs("processed_dataset/class_3", exist_ok=True)
    os.makedirs("processed_dataset/class_4", exist_ok=True)
    counts0 = 0
    counts1 = 0
    counts2 = 0
    counts3 = 0
    ...
    # `{'image': np.array(shape=(28, 28, 1)), 'labels': np.array(shape=())}`
      data, clas = ex
      if clas == 0:
        wav_data = tf.audio.encode_wav(data, sample_rate=FLAGS.sample_rate)
        # Save the encoded WAV data to a file
        file_path = f"processed_dataset/class_0/output{counts0}.wav"
        tf.io.write_file(file_path, wav_data)
        counts0 += 1
      elif clas == 1:
        wav_data = tf.audio.encode_wav(data, sample_rate=FLAGS.sample_rate)
        # Save the encoded WAV data to a file
        file_path = f"processed_dataset/class_1/output{counts1}.wav"
        tf.io.write_file(file_path, wav_data)
        counts1 += 1
      elif clas == 2:
        wav_data = tf.audio.encode_wav(data, sample_rate=FLAGS.sample_rate)
        # Save the encoded WAV data to a file
        file_path = f"processed_dataset/class_2/output{counts2}.wav"
        tf.io.write_file(file_path, wav_data)
        counts2 += 1
      elif clas == 3:
        wav_data = tf.audio.encode_wav(data, sample_rate=FLAGS.sample_rate)
        # Save the encoded WAV data to a file
        file_path = f"processed_dataset/class_3/output{counts3}.wav"
        tf.io.write_file(file_path, wav_data)
        counts3 += 1
      elif clas == 4:
        wav_data = tf.audio.encode_wav(data, sample_rate=FLAGS.sample_rate)
        # Save the encoded WAV data to a file
        file_path = f"processed_dataset/class_4/output{counts4}.wav"
        tf.io.write_file(file_path, wav_data)
        counts4 += 1
